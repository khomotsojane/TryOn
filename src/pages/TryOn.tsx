...
const TryOn: React.FC = () => {
  ...
  // GET CLOTHING FROM FEATURED PAGE
  const location = useLocation();

  useEffect(() => {
    if (location.state?.clothingImage) {
      setClothing(location.state.clothingImage);
    }
  }, [location]);
  ...
  const analyzeBody = async (img: HTMLImageElement) => {
    ...
    drawLine(landmarks[11], landmarks[23]);
    drawLine(landmarks[12], landmarks[24]);
    drawLine(landmarks[23], landmarks[24]);
    drawLine(landmarks[23], landmarks[25]);
